...
export function MyPageNicknameScreen({ username, displayName, onUpdateProfile, onCheckNickname, onGoBack }: MyPageNicknameScreenProps) {
	...
	const isCheckSatisfied = trimmedNickname.length === 0 ? true : checkStatus === "success" && validatedNickname === trimmedNickname;
				...
				<View style={styles.section}>
					...
					<View style={styles.inputRow}>
						...
						<TouchableOpacity
							...
							activeOpacity={0.85}
						>
							{checking ? <ActivityIndicator color={theme.accent} /> : <Text style={styles.checkButtonText}>중복 확인</Text>}
						</TouchableOpacity>
					</View>
					{checkMessage ? (
						<Text style={checkStatus === "success" ? styles.successText : styles.errorText}>{checkMessage}</Text>
					) : null}
					{error ? <Text style={styles.errorText}>{error}</Text> : null}
					<TouchableOpacity style={[styles.submitButton, (loading || isSubmitDisabled) && styles.submitButtonDisabled]} onPress={handleSubmit} disabled={isSubmitDisabled} activeOpacity={0.9}>
